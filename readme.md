...
4. Launch VMware, open the extracted virtual machine and power it on
5. After boot completes, log in as *root* with password *alpine*
6. Import a small test region (Russia, Kaliningrad) using command `osm import russia/kaliningrad`
...
- Regions are downloaded from https://download.geofabrik.de. You can find the exact region name from the URL of your desired region
...
- To access the virtual machine via SSH, use [PuTTY](https://www.chiark.greenend.org.uk/~sgtatham/putty/), and for SFTP use [WinSCP](https://winscp.net/eng/downloads.php). A portable "all-in-one" package can be found [here]().
